...
rpe = Robust_pol_Kl_uncertainity(nS, nA, cost_list, init_dist, alpha=0.0001)
...
df.to_excel('VF_CF_kl_lambda_Gar_CRPO_aistats_rvi_baseline_comp.xlsx')

'''with open("Value_function_kl_lambda_Gar_CRPO_aistats_rvi_baseline_comp"+str(nS)+"_"+str(nA)+"_"+str(kl_lambda)+".pkl","wb") as f:
    pickle.dump(vf,f)
f.close()

with open("Cost_function_kl_lambda_Gar_CRPO_aistats_rvi_baseline_comp"+str(nS)+"_"+str(nA)+"_"+str(kl_lambda)+".pkl","wb") as f:
    pickle.dump(cf,f)
f.close()'''
# Final policy
